...
$xmlString = '
		<xml>
			<users>
				<user id="1" username="asd" password="fgh"/>
			</users>
			<security>
				<authentication>
					<form>
...
require_once(dirname(dirname(__DIR__))."/libraries/php-security-api/src/authentication/XMLAuthentication.php");
...
$wrapper = new XMLAuthenticationWrapper($xml,"xasd", array(), $csrf);
$result = $wrapper->getResult();
echo __LINE__.":".($result===null?"OK":"FAILED")."\n";

// test login
$_POST = array("x"=>"asd","y"=>"fgh","csrf"=>$token);
$wrapper = new XMLAuthenticationWrapper($xml,"loginz", array($pd1,$pd2), $csrf);
$result = $wrapper->getResult();
...
// test logout
$wrapper = new XMLAuthenticationWrapper($xml,"logoutz", array($pd1,$pd2), $csrf);
$result = $wrapper->getResult();
